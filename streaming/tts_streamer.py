...
async def _fetch_chunk_profile(client: httpx.AsyncClient) -> ChunkProfile:
    url = _tts_url()
    try:
        resp = await client.get(f"{url}/api/v1/voice/streaming-config", timeout=2.0, headers=_auth_headers())
...
async def _send_tts_segment(
    ...
    async with _tts_send_lock:
        if is_generation_current_fn and not is_generation_current_fn():
            ...
            return
        try:
            resp = await client.post(f"{url}/api/v1/voice/playback", json=payload, timeout=30.0, headers=_auth_headers())
            logger.info(
